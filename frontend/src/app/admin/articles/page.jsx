            ...
            {filtered.map((article) => (
              <div key={article._id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col lg:flex-row gap-8 items-center group">
                ...
                <div className="w-full lg:w-48 aspect-video lg:aspect-square rounded-3xl overflow-hidden flex-shrink-0">
                   <img src={article.thumbnail || "/placeholder.jpg"} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                </div>
                ...
                <div className="flex flex-wrap gap-3 w-full lg:w-auto">
                   {article.status !== 'approved' && (
                     <button 
                       ...
                       className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-emerald-100 hover:bg-emerald-700 transition-all"
                     >
                   ...
                   {article.status !== 'rejected' && (
                     <button 
                       ...
                       className="flex-1 lg:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-rose-50 text-rose-600 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-100 transition-all"
                     >
                   ...
                   <button 
                     ...
                     className="flex-1 lg:flex-none flex items-center justify-center p-4 bg-slate-50 text-slate-400 rounded-2xl hover:bg-rose-600 hover:text-white transition-all"
                   >
                     <Trash2 size={20} />
                   </button>
                   <a 
                     ...
                     target="_blank"
                     className="flex-1 lg:flex-none flex items-center justify-center p-4 bg-slate-50 text-slate-900 rounded-2xl hover:bg-indigo-600 hover:text-white transition-all"
                   >
                     <Eye size={20} />
                   </a>
                </div>
