...
export default function FilesPage() {
    ...
    const fetchFiles = async (parentId = null) => {
        setLoading(true);
        try {
            const res = await fetch(`/api/recruiter/files?parentId=${parentId || 'null'}`);
    ...
    const handleFileUpload = async (e) => {
        ...
        try {
            ...
            // Here we simulate the URL and save metadata to DB
            const simulatedUrl = `/uploads/${file.name}`;
            
            const res = await fetch('/api/recruiter/files', {
                method: 'POST',
    ...
    const deleteItem = async (id) => {
        ...
        try {
            const res = await fetch(`/api/recruiter/files?id=${id}`, { method: 'DELETE' });
    ...
    return (
        <div className="flex min-h-screen bg-[#FDFEFF]">
            <RecruiterSidebar activePage="files" />
            ...
            <main className="flex-1 p-4 lg:p-10">
                <div className="max-w-7xl mx-auto space-y-10">
                    ...
                    <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                        ...
                        <div className="flex gap-4">
                            <button 
                                ...
                                className="flex items-center gap-2 bg-indigo-50 text-indigo-600 px-6 py-4 rounded-[24px] font-black hover:bg-indigo-100 transition-all"
                            >
                                <Folder size={20} />
                                New Folder
                            </button>
                            ...
                            <button 
                                ...
                                disabled={uploading}
                                className="flex items-center gap-2 bg-slate-900 text-white px-8 py-4 rounded-[24px] font-black shadow-xl shadow-slate-200 hover:-translate-y-1 transition-all disabled:opacity-50"
                            >
                    ...
                    {/* Storage Info */}
                    <div className="bg-slate-900 rounded-[40px] p-8 text-white flex flex-col md:flex-row items-center justify-between gap-10">
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center px-2">
                            <h3 className="text-2xl font-black text-slate-900">
                                {loading ? "Loading..." : files.length > 0 ? "Everything" : "Empty"}
                            </h3>
                        ...
                        {viewMode === "grid" ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                                {files.map((item) => (
                                    <div 
                                        key={item._id} 
                                        className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm hover:shadow-xl transition-all group cursor-pointer relative"
                            ...
                            <div className="space-y-3">
                                {files.map((item) => (
                                    <div 
                                        key={item._id} 
                                        className="bg-white p-5 rounded-3xl border border-slate-50 hover:border-indigo-100 transition-all flex items-center gap-6 group cursor-pointer"
                                        onClick={() => item.type === 'folder' ? navigateToFolder(item) : null}
                                    >
                                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${item.type === 'folder' ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-50 text-slate-400'}`}>
                                            {item.type === 'folder' ? <Folder size={20} /> : getFileIcon(item.mimetype)}
                                        </div>
                                        <div className="flex-1">
                                            <h4 className="font-bold text-slate-900">{item.name}</h4>
                                            <p className="text-xs font-bold text-slate-400">{item.type === 'folder' ? 'Folder' : formatSize(item.size)}</p>
                                        </div>
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6">
                    <div className="bg-white rounded-[40px] p-10 w-full max-w-md shadow-2xl">
                        ...
                        <input 
                            ...
                            className="w-full px-6 py-4 rounded-2xl bg-slate-50 border border-slate-100 font-bold outline-none focus:ring-2 focus:ring-indigo-600 mb-8"
                            autoFocus
                        />
                        <div className="flex gap-4">
                            <button onClick={() => setShowNewFolderModal(false)} className="flex-1 py-4 rounded-2xl font-black text-slate-400 hover:bg-slate-50 transition-all">Cancel</button>
                            <button onClick={createFolder} className="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-black shadow-xl shadow-slate-200 transition-all">Create</button>
                        </div>
                    </div>
                </div>
            )}
