...
import { HelpCircle, Send, MessageSquare, History, Plus, AlertCircle, CheckCircle2, ChevronRight, Tool, Info, Loader2, Clock, X } from 'lucide-react';
...
export default function ServiceRequestPage() {
    ...
    return (
        <div className="flex min-h-screen bg-[#FDFEFF]">
            <RecruiterSidebar activePage="service" />
                ...
                <div className="max-w-7xl mx-auto space-y-10">
                    ...
                    <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                        <div>
                            ...
                            <p className="text-slate-500 font-medium mt-1">Need help or a custom feature? Our support team is here for you.</p>
                        </div>
                    </header>
                        ...
                        <div className="lg:col-span-3 space-y-8">
                            <div className="bg-white rounded-[40px] p-8 md:p-12 border border-slate-100 shadow-sm">
                                <div className="flex items-center gap-3 mb-10">
                                    ...
                                    <h3 className="text-2xl font-black text-slate-900">New Service Request</h3>
                                </div>

                                <form onSubmit={handleRequest} className="space-y-6">
                                    <div>
                                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Request Subject</label>
                                        <input 
                                            type="text"
                                            value={subject}
                                            onChange={(e) => setSubject(e.target.value)}
                                            placeholder="E.g. Issue with job posting / Custom API Request"
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3">Detailed Message</label>
                                        <textarea 
                                            value={message}
                                            onChange={(e) => setMessage(e.target.value)}
                                            placeholder="Provide more context so we can help you better..."
                                    ...
                                    >
                                        {sending ? <><Loader2 className="animate-spin" size={20} /> Submitting...</> : <><Send size={20} /> Submit Request</>}
                                    </button>
                                </form>
                            </div>
                        </div>
                            ...
                            <div className="bg-white rounded-[40px] p-8 border border-slate-100 shadow-sm min-h-[400px]">
                                ...
                                {loading ? (
                                    ...
                                    <div className="space-y-4">
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 px-2">Click to view admin response</p>
                                        {history.map(item => (
                                            <div 
                                                key={item._id} 
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
                    <div className="bg-white rounded-[40px] w-full max-w-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                        ...
                        <div className="bg-indigo-600 p-8 md:p-10 text-white relative">
                            ...
                            <div className="space-y-4">
                                <span className="px-4 py-1.5 bg-white/20 rounded-full text-[10px] font-black uppercase tracking-[0.2em]">Service Request Details</span>
                                ...
                                <div className="flex items-center gap-4 text-indigo-100 text-xs font-bold">
                                    <span>Ref: #{selectedRequest._id.slice(-6).toUpperCase()}</span>
                                    <span>•</span>
                            ...
                            <div className="space-y-4 pt-4 border-t border-slate-100">
                                ...
                                {selectedRequest.adminReply ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center py-10 bg-slate-50 rounded-[32px] border border-dashed border-slate-200">
                                        ...
                                        <p className="text-slate-400 font-bold italic">Waiting for our team to review and respond...</p>
                                    </div>
                                )}
                            </div>
                        </div>
