...
export default function ContactsPage() {
    ...
    const fetchContacts = async () => {
        try {
            const res = await fetch('/api/recruiter/contacts', { cache: 'no-store' });
            const data = await res.json();
    ...
    const handleSaveContact = async (e) => {
        ...
        try {
            ...
            if (res.ok && data.ok) {
                ...
                setNewContact({ name: "", email: "", mobile: "", role: "", city: "", state: "", status: "Applied" });
                fetchContacts();
            } else {
    ...
    const handleDeleteContact = async (id) => {
        ...
        try {
            const res = await fetch(`/api/recruiter/contacts?id=${id}`, {
                method: 'DELETE'
            });
            const data = await res.json();
            if (res.ok && data.ok) {
    ...
    const handleImportExcel = async (e) => {
        ...
        reader.onload = async (evt) => {
            try {
                ...
                // Map columns to match recruiter contacts fields: name, email, mobile, role, city, state, status
    ...
    const filteredContacts = contacts.filter(contact => {
        ...
        const matchesQuery = 
            ...
            (contact.mobile || contact.phone || "").toLowerCase().includes(query) ||
                        ...
                        <div className="relative">
                            ...
                            {showAdvancedFilter && (
                                <div className="absolute right-0 mt-3 w-80 bg-white p-6 rounded-3xl shadow-xl border border-slate-100 z-50 space-y-4">
                                    <h4 className="font-black text-slate-800 text-sm">Filter Candidates</h4>
                                    <div>
                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Role / Profession</label>
                                        <input 
                                            type="text"
                                            value={advancedFilters.role}
                                            onChange={(e) => { setAdvancedFilters(prev => ({ ...prev, role: e.target.value })); setCurrentPage(1); }}
                                            placeholder="e.g. Developer, IT"
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {paginatedContacts.length > 0 ? paginatedContacts.map((contact) => (
                                        ...
                                        <tr>
                                            <td colSpan="7" className="px-8 py-20 text-center text-slate-400 font-bold">No contacts found in your recruitment history.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
